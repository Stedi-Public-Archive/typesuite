...
var _jsonix_factory = function (
  ...
  Jsonix.Util.Ensure = {
    ...
    ensureInteger: function (value) {
      if (!Jsonix.Util.Type.isNumber(value)) {
        throw new Error(
          "Argument [" + value + "] must be an integer, but it is not a number."
        );
  ...
  Jsonix.XML.QName = Jsonix.Class({
    ...
    toCanonicalString: function (namespaceContext) {
      var canonicalPrefix = namespaceContext
        ? namespaceContext.getPrefix(this.namespaceURI, this.prefix)
        : this.prefix;
  ...
  Jsonix.XML.QName.fromString = function (
    qNameAsString,
    namespaceContext,
    defaultNamespaceURI
  ) {
  ...
  Jsonix.XML.QName.fromObjectOrString = function (
    value,
    namespaceContext,
    defaultNamespaceURI
  ) {
    if (Jsonix.Util.Type.isString(value)) {
      return Jsonix.XML.QName.fromString(
        value,
        namespaceContext,
        defaultNamespaceURI
      );
  ...
  Jsonix.XML.Calendar = Jsonix.Class({
    ...
    initialize: function (data) {
      ...
      if (Jsonix.Util.Type.exists(data.day)) {
        ...
        if (
          Jsonix.Util.NumberUtils.isInteger(data.year) &&
          Jsonix.Util.NumberUtils.isInteger(data.month)
        ) {
          Jsonix.XML.Calendar.validateYearMonthDay(
            data.year,
            data.month,
            data.day
          );
  ...
  Jsonix.XML.Calendar.DAYS_IN_MONTH = [
    31,
    29,
    31,
    30,
  ...
  Jsonix.XML.Calendar.validateMonth = function (month) {
    if (month < 1 || month > 12) {
      throw new Error(
        "Invalid month [" + month + "]. Month must be in range [1, 12]."
      );
    }
  };
  Jsonix.XML.Calendar.validateDay = function (day) {
    if (day < 1 || day > 31) {
      throw new Error(
        "Invalid day [" + day + "]. Day must be in range [1, 31]."
      );
    }
  };
  ...
  Jsonix.XML.Calendar.validateHour = function (hour) {
    if (hour < 0 || hour > 23) {
      throw new Error(
        "Invalid hour [" + hour + "]. Hour must be in range [0, 23]."
      );
    }
  };
  Jsonix.XML.Calendar.validateMinute = function (minute) {
    if (minute < 0 || minute > 59) {
      throw new Error(
        "Invalid minute [" + minute + "]. Minute must be in range [0, 59]."
      );
    }
  };
  Jsonix.XML.Calendar.validateSecond = function (second) {
    if (second < 0 || second > 59) {
      throw new Error(
        "Invalid second [" + second + "]. Second must be in range [0, 59]."
      );
    }
  };
    ...
    if (fractionalSecond < 0 || fractionalSecond > 59) {
      throw new Error(
        "Invalid fractional second [" +
          fractionalSecond +
          "]. Fractional second must be in range [0, 1)."
      );
    }
  };
  Jsonix.XML.Calendar.validateTimezone = function (timezone) {
    if (
      timezone < Jsonix.XML.Calendar.MIN_TIMEZONE ||
      timezone > Jsonix.XML.Calendar.MAX_TIMEZONE
    ) {
      throw new Error(
        "Invalid timezone [" +
          timezone +
          "]. Timezone must not be in range [" +
          Jsonix.XML.Calendar.MIN_TIMEZONE +
          ", " +
          Jsonix.XML.Calendar.MAX_TIMEZONE +
          "]."
      );
    }
  };
  Jsonix.XML.Input = Jsonix.Class({
    ...
    retrieveElement: function () {
      ...
      if (this.eventType === 1) {
        ...
        throw new Error(
          "Element can only be retrieved for START_ELEMENT or ATTRIBUTE nodes."
        );
      }
      return element;
    },
    retrieveAttributes: function () {
      ...
      if (this.attributes) {
        ...
        throw new Error(
          "Attributes can only be retrieved for START_ELEMENT or ATTRIBUTE nodes."
        );
      }
      return attributes;
    },
      ...
      for (var i = 0, len = attributes.length; i < len; ++i) {
        ...
        if (potentialNode.namespaceURI === namespaceURI) {
          fullName = potentialNode.prefix
            ? potentialNode.prefix + ":" + localPart
            : localPart;
  ...
  Jsonix.XML.Input.prototype.getAttributeValueNS = Jsonix.DOM.isDomImplementationAvailable()
    ? Jsonix.XML.Input.prototype.getAttributeValueNSViaElement
    : Jsonix.XML.Input.prototype.getAttributeValueNSViaAttribute;
  Jsonix.XML.Input.prototype.getAttributeNodeNS = Jsonix.DOM.isDomImplementationAvailable()
    ? Jsonix.XML.Input.prototype.getAttributeNodeNSViaElement
    ...
    initialize: function (options) {
      if (Jsonix.Util.Type.exists(options)) {
        ...
        if (Jsonix.Util.Type.isString(options.mappingStyle)) {
          ...
          if (!mappingStyle) {
            throw new Error(
              "Mapping style [" + options.mappingStyle + "] is not known."
            );
          }
  ...
  Jsonix.Binding.Marshalls.Element = Jsonix.Class({
    marshalElement: function (value, context, output, scope) {
      var elementValue = this.convertToTypedNamedValue(
        value,
        context,
        output,
        scope
      ...
      if (
        context.supportXsiType &&
        Jsonix.Util.Type.exists(elementValue.value)
      ) {
      ...
      if (typeInfo) {
        ...
        if (actualTypeInfo && declaredTypeInfo !== actualTypeInfo) {
          ...
          var xsiType = Jsonix.Schema.XSD.QName.INSTANCE.print(
            xsiTypeName,
            context,
            output,
            scope
        ...
        throw new Error(
          "Element [" +
            elementValue.name.key +
            "] is not known in this context, could not determine its type."
        );
      }
    },
  ...
  Jsonix.Binding.Marshalls.Element.AsElementRef = Jsonix.Class({
    convertToTypedNamedValue: function (value, context, output, scope) {
      ...
      var elementValue = this.convertToNamedValue(
        value,
        context,
        output,
        scope
      ...
      return {
        ...
        typeInfo: this.getTypeInfoByElementName(
          elementValue.name,
          context,
          scope
        ),
    ...
    convertToNamedValue: function (elementValue, context, output, scope) {
      ...
      var value;
      if (
        Jsonix.Util.Type.exists(elementValue.name) &&
        !Jsonix.Util.Type.isUndefined(elementValue.value)
      ) {
        name = Jsonix.XML.QName.fromObjectOrString(elementValue.name, context);
        value = Jsonix.Util.Type.exists(elementValue.value)
          ? elementValue.value
          : null;
        return {
          name: name,
      ...
      throw new Error(
        "Invalid element value [" +
          elementValue +
          "]. Element values must either have {name:'myElementName', value: elementValue} or {myElementName:elementValue} structure."
      );
    },
  });
  ...
  Jsonix.Binding.Unmarshalls.Element = Jsonix.Class({
    ...
    unmarshalElement: function (context, input, scope, callback) {
      if (input.eventType != 1) {
        throw new Error(
          "Parser must be on START_ELEMENT to read next element."
        );
      }
      ...
      if (this.allowTypedObject) {
        if (Jsonix.Util.Type.exists(typeInfo)) {
          ...
          elementValue = this.convertFromTypedNamedValue(
            typedNamedValue,
            context,
            input,
            scope
        ...
        throw new Error(
          "Element [" +
            name.toString() +
            "] could not be unmarshalled as the property neither allows typed objects nor DOM as content. This is a sign of invalid mappings, do not use [allowTypedObject : false] and [allowDom : false] at the same time."
        );
      }
    ...
    getTypeInfoByInputElement: function (context, input, scope) {
      ...
      if (context.supportXsiType) {
        var xsiType = input.getAttributeValueNS(
          Jsonix.Schema.XSI.NAMESPACE_URI,
          Jsonix.Schema.XSI.TYPE
        );
        if (Jsonix.Util.StringUtils.isNotBlank(xsiType)) {
          var xsiTypeName = Jsonix.Schema.XSD.QName.INSTANCE.parse(
            xsiType,
            context,
            input,
            scope
      ...
      var name = input.getName();
      var typeInfo = xsiTypeInfo
        ? xsiTypeInfo
        : this.getTypeInfoByElementName(name, context, scope);
      return typeInfo;
    },
    getTypeInfoByElementName: function (name, context, scope) {
      var elementInfo = context.getElementInfo(name, scope);
  ...
  Jsonix.Binding.Unmarshalls.Element.AsElementRef = Jsonix.Class({
    convertFromTypedNamedValue: function (
      typedNamedValue,
      context,
      input,
      scope
  ...
  Jsonix.Binding.Marshaller = Jsonix.Class(
    Jsonix.Binding.Marshalls.Element,
    Jsonix.Binding.Marshalls.Element.AsElementRef,
    {
      context: null,
  ...
  Jsonix.Binding.Marshaller.Simplified = Jsonix.Class(
    Jsonix.Binding.Marshaller,
    {
      CLASS_NAME: "Jsonix.Binding.Marshaller.Simplified",
    }
  );
  Jsonix.Binding.Unmarshaller = Jsonix.Class(
    Jsonix.Binding.Unmarshalls.Element,
    Jsonix.Binding.Unmarshalls.Element.AsElementRef,
    {
      context: null,
      allowTypedObject: true,
      allowDom: false,
  ...
  Jsonix.Binding.Unmarshaller.Simplified = Jsonix.Class(
    Jsonix.Binding.Unmarshaller,
    Jsonix.Binding.Unmarshalls.Element.AsSimplifiedElementRef,
    {
  ...
  Jsonix.Model.ClassInfo = Jsonix.Class(
    Jsonix.Model.TypeInfo,
    Jsonix.Mapping.Styled,
    {
      name: null,
      localName: null,
      typeName: null,
      ...
      build: function (context) {
        if (!this.built) {
          this.baseTypeInfo = context.resolveTypeInfo(
            this.baseTypeInfo,
            this.module
          );
          ...
          while (et !== Jsonix.XML.Input.END_ELEMENT) {
            if (et === Jsonix.XML.Input.START_ELEMENT) {
              ...
              if (
                ...
                var elementPropertyInfo = this.structure.elements[
                  elementNameKey
                ];
        ...
        if (this.isMarshallable(value, context, scope)) {
          ...
          if (this.structure.value) {
            ...
            throw new Error(
              "The passed value [" +
                value +
                "] is not an object and there is no single suitable property to marshal it."
            );
          }
        }
      },
      ...
      isMarshallable: function (value, context, scope) {
        return (
          ...
          (Jsonix.Util.Type.isObject(value) && !Jsonix.Util.Type.isArray(value))
        );
      },
      ...
      addDefaultNamespaces: function (mapping) {
        if (Jsonix.Util.Type.isObject(mapping)) {
          ...
          if (
            !Jsonix.Util.Type.isString(mapping.defaultAttributeNamespaceURI)
          ) {
  ...
  Jsonix.Model.AnyAttributePropertyInfo = Jsonix.Class(
    Jsonix.Model.PropertyInfo,
    {
      ...
      unmarshal: function (context, input, scope) {
        ...
        if (attributeCount === 0) {
          ...
          for (var index = 0; index < attributeCount; index++) {
            ...
            if (Jsonix.Util.Type.isString(value)) {
              var propertyName = this.convertFromAttributeName(
                input.getAttributeName(index),
                context,
                input,
                scope
              );
        ...
        for (var propertyName in value) {
          if (value.hasOwnProperty(propertyName)) {
            ...
            if (Jsonix.Util.Type.isString(propertyValue)) {
              var attributeName = this.convertToAttributeName(
                propertyName,
                context,
                output,
                scope
      ...
      convertFromAttributeName: function (
        attributeName,
        context,
        input,
        scope
  ...
  Jsonix.Model.AnyAttributePropertyInfo.Simplified = Jsonix.Class(
    Jsonix.Model.AnyAttributePropertyInfo,
    {
      convertFromAttributeName: function (
        attributeName,
        context,
  ...
  Jsonix.Model.AbstractElementsPropertyInfo = Jsonix.Class(
    Jsonix.Binding.Unmarshalls.Element,
    Jsonix.Binding.Unmarshalls.WrapperElement,
    Jsonix.Model.PropertyInfo,
    {
    ...
    Jsonix.Binding.Marshalls.Element,
    {
      ...
      initialize: function (mapping) {
        ...
        for (var index = 0; index < etis.length; index++) {
          this.elementTypeInfos[index] = Jsonix.Util.Type.cloneObject(
            etis[index]
          );
        }
      },
      ...
      doBuild: function (context, module) {
        ...
        for (var index = 0; index < this.elementTypeInfos.length; index++) {
          ...
          elementTypeInfo.elementName = Jsonix.XML.QName.fromObjectOrString(
            etien,
            context,
            this.defaultElementNamespaceURI
          );
  ...
  Jsonix.Model.AbstractElementRefsPropertyInfo = Jsonix.Class(
    ...
    Jsonix.Model.PropertyInfo,
    {
      ...
      initialize: function (mapping) {
        ...
        var dom = Jsonix.Util.Type.defaultValue(
          mapping.allowDom,
          mapping.dom,
          true
        );
        var typed = Jsonix.Util.Type.defaultValue(
          mapping.allowTypedObject,
          mapping.typed,
          true
        );
  ...
  Jsonix.Model.ElementRefPropertyInfo.Simplified = Jsonix.Class(
    Jsonix.Model.ElementRefPropertyInfo,
    Jsonix.Binding.Unmarshalls.Element.AsSimplifiedElementRef,
    {
  ...
  Jsonix.Model.ElementRefsPropertyInfo.Simplified = Jsonix.Class(
  ...
  Jsonix.Model.AnyElementPropertyInfo.Simplified = Jsonix.Class(
  ...
  Jsonix.Schema.XSD.Calendar = Jsonix.Class(Jsonix.Schema.XSD.AnySimpleType, {
    ...
    parse: function (text, context, input, scope) {
      ...
      if (
        text.match(
          new RegExp("^" + Jsonix.Schema.XSD.Calendar.DATETIME_PATTERN + "$")
        )
      ) {
        return this.parseDateTime(text, context, input, scope);
      } else if (
        text.match(
          new RegExp("^" + Jsonix.Schema.XSD.Calendar.DATE_PATTERN + "$")
        )
      ) {
        return this.parseDate(text, context, input, scope);
      } else if (
        text.match(
          new RegExp("^" + Jsonix.Schema.XSD.Calendar.TIME_PATTERN + "$")
        )
      ) {
        return this.parseTime(text, context, input, scope);
      } else if (
        text.match(
          new RegExp("^" + Jsonix.Schema.XSD.Calendar.GYEAR_MONTH_PATTERN + "$")
        )
      ) {
        return this.parseGYearMonth(text, context, input, scope);
      } else if (
        text.match(
          new RegExp("^" + Jsonix.Schema.XSD.Calendar.GYEAR_PATTERN + "$")
        )
      ) {
        return this.parseGYear(text, context, input, scope);
      } else if (
        text.match(
          new RegExp("^" + Jsonix.Schema.XSD.Calendar.GMONTH_DAY_PATTERN + "$")
        )
      ) {
        return this.parseGMonthDay(text, context, input, scope);
      } else if (
        text.match(
          new RegExp("^" + Jsonix.Schema.XSD.Calendar.GMONTH_PATTERN + "$")
        )
      ) {
        return this.parseGMonth(text, context, input, scope);
      } else if (
        text.match(
          new RegExp("^" + Jsonix.Schema.XSD.Calendar.GDAY_PATTERN + "$")
        )
      ) {
        ...
        throw new Error(
          "Value [" +
            text +
            "] does not match xs:dateTime, xs:date, xs:time, xs:gYearMonth, xs:gYear, xs:gMonthDay, xs:gMonth or xs:gDay patterns."
        );
      }
    },
    parseGYearMonth: function (value, context, input, scope) {
      var gYearMonthExpression = new RegExp(
        "^" + Jsonix.Schema.XSD.Calendar.GYEAR_MONTH_PATTERN + "$"
      );
      ...
      throw new Error(
        "Value [" + value + "] does not match the xs:gYearMonth pattern."
      );
    },
    parseGYear: function (value, context, input, scope) {
      var gYearExpression = new RegExp(
        "^" + Jsonix.Schema.XSD.Calendar.GYEAR_PATTERN + "$"
      );
      ...
      throw new Error(
        "Value [" + value + "] does not match the xs:gYear pattern."
      );
    },
    parseGMonthDay: function (value, context, input, scope) {
      var gMonthDayExpression = new RegExp(
        "^" + Jsonix.Schema.XSD.Calendar.GMONTH_DAY_PATTERN + "$"
      );
      ...
      throw new Error(
        "Value [" + value + "] does not match the xs:gMonthDay pattern."
      );
    },
    parseGMonth: function (value, context, input, scope) {
      var gMonthExpression = new RegExp(
        "^" + Jsonix.Schema.XSD.Calendar.GMONTH_PATTERN + "$"
      );
      ...
      throw new Error(
        "Value [" + value + "] does not match the xs:gMonth pattern."
      );
    },
    parseGDay: function (value, context, input, scope) {
      var gDayExpression = new RegExp(
        "^" + Jsonix.Schema.XSD.Calendar.GDAY_PATTERN + "$"
      );
      ...
      throw new Error(
        "Value [" + value + "] does not match the xs:gDay pattern."
      );
    },
    parseDateTime: function (text, context, input, scope) {
      Jsonix.Util.Ensure.ensureString(text);
      var expression = new RegExp(
        "^" + Jsonix.Schema.XSD.Calendar.DATETIME_PATTERN + "$"
      );
      ...
      throw new Error(
        "Value [" + text + "] does not match the xs:date pattern."
      );
    },
    parseDate: function (text, context, input, scope) {
      Jsonix.Util.Ensure.ensureString(text);
      var expression = new RegExp(
        "^" + Jsonix.Schema.XSD.Calendar.DATE_PATTERN + "$"
      );
      var results = text.match(expression);
      if (results !== null) {
        var data = {
    ...
    parseTime: function (text, context, input, scope) {
      Jsonix.Util.Ensure.ensureString(text);
      var expression = new RegExp(
        "^" + Jsonix.Schema.XSD.Calendar.TIME_PATTERN + "$"
      );
      var results = text.match(expression);
      if (results !== null) {
        var data = {
      ...
      throw new Error(
        "Value [" + text + "] does not match the xs:time pattern."
      );
    },
    parseTimezoneString: function (text) {
      ...
      if (!Jsonix.Util.Type.isString(text)) {
        ...
        var expression = new RegExp(
          "^" + Jsonix.Schema.XSD.Calendar.TIMEZONE_PATTERN + "$"
        );
        var results = text.match(expression);
        if (results !== null) {
        ...
        throw new Error(
          "Value [" + text + "] does not match the timezone pattern."
        );
      }
    },
    print: function (value, context, output, scope) {
    ...
    printGMonthDay: function (value, context, output, scope) {
      ...
      return (
        ...
        "-" +
        this.printDay(day) +
        this.printTimezoneString(timezone)
      );
    },
  ...
  Jsonix.Schema.XSD.Calendar.YEAR_PATTERN =
    "-?([1-9][0-9]*)?((?!(0000))[0-9]{4})";
  Jsonix.Schema.XSD.Calendar.TIMEZONE_PATTERN =
    "Z|([\\-\\+])(((0[0-9]|1[0-3]):([0-5][0-9]))|(14:00))";
  Jsonix.Schema.XSD.Calendar.MONTH_PATTERN = "(0[1-9]|1[0-2])";
  Jsonix.Schema.XSD.Calendar.SINGLE_MONTH_PATTERN =
    "\\-\\-" + Jsonix.Schema.XSD.Calendar.MONTH_PATTERN;
  Jsonix.Schema.XSD.Calendar.DAY_PATTERN = "(0[1-9]|[12][0-9]|3[01])";
  Jsonix.Schema.XSD.Calendar.SINGLE_DAY_PATTERN =
    "\\-\\-\\-" + Jsonix.Schema.XSD.Calendar.DAY_PATTERN;
  Jsonix.Schema.XSD.Calendar.GYEAR_PATTERN =
    "(" +
    Jsonix.Schema.XSD.Calendar.YEAR_PATTERN +
    ")" +
  ...
  Jsonix.Schema.XSD.Calendar.GMONTH_DAY_PATTERN =
    ...
    Jsonix.Schema.XSD.Calendar.TIMEZONE_PATTERN +
    ")?";
  Jsonix.Schema.XSD.Calendar.DATE_PART_PATTERN =
    "(" +
    Jsonix.Schema.XSD.Calendar.YEAR_PATTERN +
    ")" +
    "-" +
    ...
    Jsonix.Schema.XSD.Calendar.DAY_PATTERN +
    ")";
  Jsonix.Schema.XSD.Calendar.TIME_PART_PATTERN =
  ...
  Jsonix.Schema.XSD.Duration = Jsonix.Class(Jsonix.Schema.XSD.AnySimpleType, {
    ...
    isInstance: function (value, context, scope) {
      return (
        Jsonix.Util.Type.isObject(value) &&
        ((Jsonix.Util.Type.exists(value.sign)
          ? value.sign === -1 || value.sign === 1
          : true)(
          Jsonix.Util.NumberUtils.isInteger(value.years) && value.years >= 0
        ) ||
          (Jsonix.Util.NumberUtils.isInteger(value.months) &&
            value.months >= 0) ||
          (Jsonix.Util.NumberUtils.isInteger(value.days) && value.days >= 0) ||
          (Jsonix.Util.NumberUtils.isInteger(value.hours) &&
            value.hours >= 0) ||
          (Jsonix.Util.NumberUtils.isInteger(value.minutes) &&
            value.minutes >= 0) ||
    ...
    validate: function (value) {
      ...
      empty =
        empty &&
        !ifExistsEnsureUnsignedInteger(
          value.years,
          "Number of years [{0}] must be an unsigned integer."
        );
      empty =
        empty &&
        !ifExistsEnsureUnsignedInteger(
          value.months,
          "Number of months [{0}] must be an unsigned integer."
        );
      empty =
        empty &&
        !ifExistsEnsureUnsignedInteger(
          value.days,
          "Number of days [{0}] must be an unsigned integer."
        );
      empty =
        empty &&
        !ifExistsEnsureUnsignedInteger(
          value.hours,
          "Number of hours [{0}] must be an unsigned integer."
        );
      empty =
        empty &&
        !ifExistsEnsureUnsignedInteger(
          value.minutes,
          "Number of minutes [{0}] must be an unsigned integer."
        );
      empty =
        empty &&
        !ifExistsEnsureUnsignedNumber(
          value.seconds,
          "Number of seconds [{0}] must be an unsigned number."
        );
      if (empty) {
        throw new Error(
          "At least one of the components (years, months, days, hours, minutes, seconds) must be set."
        );
      }
    },
    print: function (value, context, output, scope) {
      ...
      if (
        Jsonix.Util.Type.exists(value.hours) ||
        Jsonix.Util.Type.exists(value.minutes) ||
        Jsonix.Util.Type.exists(value.seconds)
      ) {
        result += "T";
    ...
    parse: function (value, context, input, scope) {
      var durationExpression = new RegExp(
        "^" + Jsonix.Schema.XSD.Duration.PATTERN + "$"
      );
      ...
      if (results !== null) {
        ...
        throw new Error(
          "Value [" + value + "] does not match the duration pattern."
        );
      }
    },
  ...
  Jsonix.Schema.XSD.DateTimeAsDate = Jsonix.Class(Jsonix.Schema.XSD.Calendar, {
    ...
    parse: function (value, context, input, scope) {
      ...
      //
      var result = new Date(
        date.getTime() + 60000 * (-timezone + localTimezone)
      );
      if (unknownTimezone) {
      ...
      if (value.originalTimezone === null) {
        ...
        if (Jsonix.Util.NumberUtils.isInteger(value.originalTimezone)) {
          ...
          correctedValue = new Date(
            value.getTime() - 60000 * (-timezone + localTimezone)
          );
        }
  ...
  Jsonix.Schema.XSD.DateTimeAsDate.INSTANCE.LIST = new Jsonix.Schema.XSD.List(
    ...
    print: function (value, context, output, scope) {
      ...
      if (value.originalTimezone === null) {
        ...
        if (correctedTime >= -localTimezone * 60000) {
          ...
          var correctedTimeInSeconds =
            correctedValue.getSeconds() +
  ...
  Jsonix.Schema.XSD.TimeAsDate.INSTANCE.LIST = new Jsonix.Schema.XSD.List(
      ...
      if (value.originalTimezone === null) {
        ...
        if (Jsonix.Util.NumberUtils.isInteger(value.originalTimezone)) {
          var correctedValue = new Date(
            value.getTime() -
              60000 * (-value.originalTimezone - value.getTimezoneOffset())
          );
          return this.printDate(
            new Jsonix.XML.Calendar({
              year: correctedValue.getFullYear(),
  ...
  Jsonix.Schema.XSD.DateAsDate.INSTANCE.LIST = new Jsonix.Schema.XSD.List(
  ...
  Jsonix.Schema.XSI.TYPE_QNAME = Jsonix.Schema.XSI.qname(
    Jsonix.Schema.XSI.TYPE
  );
  ...
  Jsonix.Context = Jsonix.Class(Jsonix.Mapping.Styled, {
    ...
    initialize: function (mappings, options) {
      ...
      if (Jsonix.Util.Type.exists(options)) {
        Jsonix.Util.Ensure.ensureObject(options);
        if (Jsonix.Util.Type.isObject(options.namespacePrefixes)) {
          this.namespacePrefixes = Jsonix.Util.Type.cloneObject(
...
// REMOVED - amdefine does not work with esbuild
// SEE https://github.com/Stedi/integration-plugin-netsuite/issues/665#issuecomment-737282392
...
export const Jsonix = _jsonix_factory(xmldom, xmlhttprequest, fs);
